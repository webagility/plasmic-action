...
@@ -502194,6 +502194,7 @@ var PlasmicApi = class {
   // If rethrowAppError is true, we will throw an exception with the error
   // message
   async post(url2, data, rethrowAppError) {
+    console.log('__DEV__:post', { url2, data: { projectIdsAndTokens: this.projectIdsAndTokens, ...data }, headers: this.makeHeaders() });
     try {
       return await import_axios.default.post(
         url2,
@@ -502215,6 +502216,7 @@ var PlasmicApi = class {
     }
   }
   async get(url2, rethrowAppError, extraHeaders) {
+    console.log('__DEV__:get', { url2, headers: { ...this.makeHeaders(), ...extraHeaders != null ? extraHeaders : {} } });
     try {
       return await import_axios.default.get(url2, {
         headers: { ...this.makeHeaders(), ...extraHeaders != null ? extraHeaders : {} }
@@ -502269,11 +502271,14 @@ var PlasmicApi = class {
...
+      host = "https://api.squirrel.dev";
     } else {
-      return this.auth.host;
+      host = this.auth.host;
     }
+    console.log('__DEV__:codegenHost', { auth: this.auth, host, DEFAULT_HOST });
+    return host;
   }
 };
 
@@ -502450,7 +502455,9 @@ function failAuth() {
   );
 }
 async function getOrStartAuth(opts) {
+  console.log('__DEV__:getOrStartAuth', { opts });
   let auth2 = await getCurrentAuth(opts.auth);
+  console.log('__DEV__:getOrStartAuth', { auth2 });
   if (!auth2 && opts.enableSkipAuth) {
     return;
   }
@@ -503432,7 +503439,7 @@ async function getContext(args, {
 } = {}) {
   if (!args.baseDir)
...
@@ -503480,12 +503487,13 @@ async function getContext(args, {
   return context;
 }
...
@@ -505802,6 +505810,10 @@ function configureSyncArgs(yags, includeQuietOption = true) {
...
index 058fe45..c2bc426 100644
--- a/node_modules/@plasmicapp/cli/src/api.ts
+++ b/node_modules/@plasmicapp/cli/src/api.ts
@@ -455,6 +455,7 @@ export class PlasmicApi {
   // If rethrowAppError is true, we will throw an exception with the error
   // message
   private async post(url: string, data?: any, rethrowAppError?: boolean) {
+    console.log('__DEV__:post', { url, data: { projectIdsAndTokens: this.projectIdsAndTokens, ...data }, headers: this.makeHeaders() });
     try {
       return await axios.post(
         url,
@@ -480,6 +481,7 @@ export class PlasmicApi {
   }
 
   private async get(url: string, rethrowAppError?: boolean, extraHeaders?: {}) {
+    console.log('__DEV__:get', { url, headers: { ...this.makeHeaders(), ...(extraHeaders ?? {}) } });
     try {
       return await axios.get(url, {
         headers: { ...this.makeHeaders(), ...(extraHeaders ?? {}) },
@@ -544,10 +546,13 @@ export class PlasmicApi {
   }
...
+  console.log('__DEV__:getOrStartAuth', { auth });
