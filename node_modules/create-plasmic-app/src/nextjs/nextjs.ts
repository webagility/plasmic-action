...
import { makePlasmicAuth } from "./templates/plasmic-auth";
...
export const nextjsStrategy: CPAStrategy = {
  ...
  installDeps: async (args) => {
    const { scheme, projectPath } = args;
    await installPatchPackage(args);
    if (scheme === "loader") {
...
async function installPatchPackage(args: InstallArgs) {
  const { projectPath } = args;

  await generatePatchFiles(args);

  const packageJsonFile = path.join(projectPath, "package.json");
  const packageJson = await require(packageJsonFile);
  ...
  await installUpgrade("patch-package", { 
    workingDir: projectPath,
  });
};

async function generatePatchFiles(args: InstallArgs) {
  const { projectPath } = args;
...
async function generateAuthFile(args: GenerateFilesArgs) {
  const { projectPath, projectHost } = args;

  // ./.plasmic.auth
  await fs.writeFile(
    path.join(projectPath, ".plasmic.auth"),
    makePlasmicAuth(projectHost)
  );
};
